...
class PasswordField extends Component {
    ...
    validatePasswordStrong = (value) => {
        if (value.length <= this.thresholdLength) throw new Error("Entered Password is too short!");

        if (zxcvbn(value).score < this.minStrength) throw new Error("Entered Password is weak!");
    }
            ...
            <Fragment>
                <div className="position-relative">
                    <FormField type="password" validator={this.validatePasswordStrong} onStateChanged={this.stateChanged} {...restProps}>
                        <span className="d-block form-hint">Your privacy is extremely important to us, in order to ensure overal security of your account we require a password with more than 8 characters.</span>
                        {children}
